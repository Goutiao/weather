...
import { getWeekWeather, getNowTemp, getAir, getHours } from "@/request/api";
...
const getLoca = () => {
  ...
  getIp(ip).then((res) => {
    ...
    getCityGeo(city.value).then((res) => {
      ...
      areacode.value = locate.value;
      getNowTemp(locate.value).then((res) => {
        data.realtime = res.data.result.realtime;
        data.location = res.data.result.location;
      });

      getAirData();
      getWeekWeather(locate.value, "3").then((res) => {
